<<<<<<< HEAD
=======
bool isHeap(const Node* root)//assumes is complete tree
{
    ...
    else
    {
        if (root->value >= root->leftChild->value)
        {
            if (root->rightChild == NULL)
                return true;//root has single left child that has less or equal value
            else if (root->value >= root->rightChild->value)
                return isHeap(root->leftChild) && isHeap(root->rightChild);
            else
                return false;//right child value is greater than root
        }
        else
        {
            return false;//left child has greater value
        }
    }
}

>>>>>>> f341a5ffd12d3bb5ccc3a1a6065a5f57d003bf62
#include <iostream>
using namespace std;
